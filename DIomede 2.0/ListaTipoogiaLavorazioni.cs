...
namespace Diomede2
{
    public partial class ListaTipoogiaLavorazioni : Form
    {
        ...
        {
            var iL = new InserisciTipologiaLavorazione(db);
            iL.Show();
        }
            ...
            foreach (DataGridViewRow riga in dataGridView1.Rows)
                if (riga.Cells[0].Value != null)
                    if (riga.Cells[0].Style.ForeColor == Color.Red)
                        try
                        {
                            op.UpdateTipologiaLavorazione((int) riga.Cells["ID"].Value, riga.Cells["NOME"].Value + "",
                                riga.Cells["DESCRIZIONE"].Value + "", (double) riga.Cells["PREZZO"].Value,
                                riga.Cells["SCADENZE"].Value + "", (int) riga.Cells["MACROLAVORAZIONE"].Value);
                        }
                        catch
                        {
        ...
        private void Button3_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.SelectedRows != null)
                    ...
                    {
                        var clienti =
                            op.CercaTipologiaLavorazione((int) dataGridView1.Rows[dataGridView1.SelectedRows[0].Index]
                                .Cells[0].Value);
                        op.CancellaTipologiaLavorazione((int) dataGridView1.Rows[dataGridView1.SelectedRows[0].Index]
                            .Cells[0].Value);
                        MessageBox.Show("Tipologia Eliminato", "Conferma", MessageBoxButtons.OK,
        ...
        private void DataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 5)
                if (e.RowIndex != -1)
                {
                    var frm = new VisualizzatoreMacrolavorazioni(db, dataGridView1, e.RowIndex);
                    frm.Show();
                }
        }
